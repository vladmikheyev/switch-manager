...
export const exportToXLSX = (data, filename = 'export', sheetName = 'Данные') => {
  try {
    ...
    if (typeof window.XLSX === 'undefined') {
      console.warn('Библиотека XLSX не найдена. Используем CSV fallback.');
    ...
    window.XLSX.writeFile(wb, `${filename}-${getDateString()}.xlsx`);
  } catch (error) {
    console.error('Ошибка экспорта в XLSX:', error);
    throw new Error('Не удалось экспортировать данные в XLSX');
  }
};
